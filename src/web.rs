...
pub trait RequestClosure<'req, Fut>: Fn(&mut Request, &mut Resolution) -> Fut
where
    Fut: RequestFuture,
{
}
impl<'req, Fut, T> RequestClosure<'req, Fut> for T
where
    Fut: RequestFuture,
...
pub trait MiddlewareClosure<'req, Fut>: Fn(&mut Request, &mut Resolution) -> Fut
where
    Fut: MiddlewareFuture {
    
}
impl<'req, Fut, T> MiddlewareClosure<'req, Fut> for T
where
    Fut: MiddlewareFuture,
    T: Fn(&mut Request, &mut Resolution) -> Fut,
{
...
pub type ArcMiddlewareClosure = Arc<dyn Fn(&mut Request, &mut Resolution) -> PinnedMiddlewareFuture>;